...
func _open_stash_ui() -> void:
	...
	InventoryManager.open_external_inventory(InventoryManager.stash.bag, get_path())
	...
	if InventoryManager.has_signal("stash_opened"):
		InventoryManager.emit_signal("stash_opened", true)

func request_remove_item(slot_index: int) -> void:
	# This is called by move_item when moving FROM external to player.
	# For stash, InventoryManager handles it internally since it's just another InventoryData.
	pass
